...
#            return writer.layout.use_name
            ...
            #print("use_name:",writer.layout.use_name,writer.layout.use_name.rfind("/",0,lastslash-1),lastslash)
            #print("new_use_name:",writer.layout.use_name[writer.layout.use_name.rfind("/",0,lastslash-1)+1:])
...
#        return [m.make_layout(x, name).use_name for x in getattr(self.conf, what)]
        ...
        for x in getattr(self.conf, what):
          orgname = m.make_layout(x,name).use_name
          #print("orgname: ",orgname)
          if "MPI" in orgname:
            tempstr = orgname.split('/')
            myname = tempstr[6]+'/'+tempstr[7]
          else:
            myname = re.sub(r'^.*?/', '/', orgname)[1:]
          #print("WARNING - changing dependent module from {1} to {2}".format(orgname,myname))
